...
async function sendAlertEmail(userEmail: string, userName: string, listing: any, metrics: any): Promise<boolean> {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: userEmail,
      subject: `🏠 New Deal Match — ${listing.city}, ${listing.state} | Score ${metrics.dealScore}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: #1B3A6B; padding: 24px; border-radius: 8px 8px 0 0;">
            <h1 style="color: #C9A84C; margin: 0; font-size: 20px;">New Deal Match Found</h1>
          ...
          <div style="background: #fff; border: 1px solid #ddd; border-top: none; padding: 24px; border-radius: 0 0 8px 8px;">
            <p style="color: #333; font-size: 15px;">Hey ${userName}, a new listing just hit that matches your buy box:</p>
            <div style="background: #f4f1eb; border-radius: 8px; padding: 20px; margin: 16px 0;">
              <p style="margin: 0 0 4px; font-size: 18px; font-weight: 700; color: #1B3A6B;">${listing.address}</p>
              <p style="margin: 0 0 16px; color: #888; font-size: 14px;">${listing.city}, ${listing.state}</p>
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td style="padding: 8px 0; border-bottom: 1px solid #e8e4dc; font-size: 14px; color: #555;">List Price</td>
                  <td style="padding: 8px 0; border-bottom: 1px solid #e8e4dc; font-size: 14px; font-weight: 700; text-align: right;">$${listing.price?.toLocaleString()}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; border-bottom: 1px solid #e8e4dc; font-size: 14px; color: #555;">Est. Rent</td>
                  <td style="padding: 8px 0; border-bottom: 1px solid #e8e4dc; font-size: 14px; font-weight: 700; text-align: right;">$${listing.rent_estimate?.toLocaleString()}/mo</td>
                </tr>
                <tr>
            ...
            <div style="text-align: center; margin: 24px 0;">
              <a href="https://www.rental-analyst.com" style="background: #1B3A6B; color: #fff; padding: 12px 28px; border-radius: 6px; text-decoration: none; font-weight: 700; font-size: 14px;">Analyze This Deal →</a>
            </div>
...
Deno.serve(async (req) => {
  try {
    ...
    for (const { listing, metrics, market, rentEstimate } of newListingsFound) {
      for (const criteria of criteriaList) {
        if (!criteria.markets?.includes(market)) continue
        if (criteria.max_price && listing.price > criteria.max_price) continue
        if (criteria.min_coc && metrics.coc < criteria.min_coc) continue
        if (criteria.property_types && !criteria.property_types.includes('any')) {
          if (!criteria.property_types.includes(listing.propertyType)) continue
        }
        if (metrics.dealScore < 70) continue

        const userInfo = userMap[criteria.user_id]
if (userInfo?.email) {
  const [listingCity, listingState] = market.split(', ')
  const sent = await sendAlertEmail(userInfo.email, userInfo.name, {
    address: listing.formattedAddress,
    city: listingCity,
    state: listingState,
    price: listing.price,
